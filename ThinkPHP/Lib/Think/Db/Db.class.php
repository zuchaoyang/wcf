...
class Db extends Think
{
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    public function factory($alias, $master = false)
    {
        // 读取数据库配置
        $db_config = $this->parseConfig($alias);
        ...
        if(class_exists($dbClass)) {
            ...
            $db->initConnect($master);
            // 获取当前的数据库类型
            if( 'pdo' != strtolower($db_config['dbms']) )
                $db->dbType = strtoupper($this->dbType);
            else
                $db->dbType = $this->_getDsnType($db_config[$master]['dsn']);
            if(C('APP_DEBUG'))
                $db->debug    = true;
        }else {
            // 类没有定义
     ...
     * @return string
     +----------------------------------------------------------
     */
    private function parseConfig($alias) {
        $db_config = self::getDbConf($alias);
        if(!$db_config) return false;
        foreach($db_config as $key => $db) {
            $result[$key] = array (
                    'username'  =>   $db['user'],
    ...
    protected function multiConnect($master=false) {
        ...
        $db_config = array(
            'username'  =>   isset($_config[$r]['username'])?$_config[$r]['username']:$_config[0]['username'],
            'password'   =>   isset($_config[$r]['password'])?$_config[$r]['password']:$_config[0]['password'],
            'hostname'  =>   isset($_config[$r]['hostname'])?$_config[$r]['hostname']:$_config[0]['hostname'],
            'hostport'    =>   isset($_config[$r]['hostport'])?$_config[$r]['hostport']:$_config[0]['hostport'],
            'database'   =>   isset($_config[$r]['database'])?$_config[$r]['database']:$_config[0]['database'],
            'dsn'          =>   isset($_config[$r]['dsn'])?$_config[$r]['dsn']:$_config[0]['dsn'],
            'params'     =>   isset($_config[$r]['params'])?$_config[$r]['params']:$_config[0]['params'],
        );
        $linknum = $_config[$r]['alias'] . intval($master);
        return $this->connect($db_config, $linknum);
    }
    ...
    public static function getDbConf($alias) {
       $dbinfo = C('DB_INFO'); 
       $alias = strtolower($alias);
       //var_dump($alias);
       //var_dump($dbinfo[$alias]);exit;
       return isset($dbinfo[$alias]) ? $dbinfo[$alias] : false;
    }
}//类定义结束
?>
