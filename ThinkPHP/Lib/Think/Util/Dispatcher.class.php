...
{//类定义开始
    ...
    static public function dispatch()
    {
        ...
        if(!self::routerCheck() && !self::groupCheck()){   // 检测路由规则 如果没有则按默认规则调度URL
  ...
  /**
   * +-----------------------------------------------------------
   * 判断依据:检测url中path路径中对应的:group/sub_group目录是否存在
   * +-----------------------------------------------------------
   */
    static private function groupCheck() {
        $depr = C('URL_PATHINFO_DEPR');
        $paths = explode($depr,trim($_SERVER['PATH_INFO'],'/'));
        ...
        if (C('APP_GROUP_LIST') && !isset($_GET[C('VAR_GROUP')])){
            ...
            if(!isset($_GET['sub_group'])) {
                $var['sub_group'] = array_shift($paths);
            }
        }
        if(!isset($_GET[C('VAR_MODULE')])) {// 还没有定义模块名称
            $var[C('VAR_MODULE')]  =   array_shift($paths);
        ...
        //判断文件是否存在,以此来确定是否是子分组
        $file_dir = WEB_ROOT_DIR . "/Control/" . $var[C('VAR_GROUP')] . "/" . $var['sub_group'];
        $action_file = $file_dir . '/' . $var[C('VAR_MODULE')] . 'Action.class.php';
        $api_file = $file_dir . '/' . $var[C('VAR_MODULE')] . 'Api.class.php';
        
        if(is_file($action_file) || is_file($api_file)) {
            define('SUB_GROUP_NAME', $var['sub_group']);
            // 解析剩余的URL参数
            $res = preg_replace('@(\w+)'.$depr.'([^'.$depr.'\/]+)@e', '$var[\'\\1\']="\\2";', implode($depr,$paths));
            $_GET   =  array_merge($var,$_GET);
