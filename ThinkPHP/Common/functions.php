...
function __autoload($name) {
    ...
    if (substr($name, -6) == "Action") {
        require_cache(WEB_ROOT_DIR . '/Control/' . $name . '.class.php');
    } else {
        // 根据自动加载路径设置进行尝试搜索
...
function import($class, $baseUrl = '', $ext='.class.php') {
    ...
    $libs = 'libraries';
    $isRoot = in_array(strtolower($class_strut[0]), array('common', 'config', 'control', 'data', $libs, 'model', 'rdata', 'rmodel'));
    if (empty($baseUrl)) {
        if ('@' == $class_strut[0] || APP_NAME == $class_strut[0] || $isRoot) {
            //加载当前项目应用类库
            $baseUrl = WEB_ROOT_DIR;
            if($isRoot && strtolower($class_strut[0]) == $libs) $baseUrl = WCF_PATH;
            if(!$isRoot) $class = substr_replace($class, '', 0, strlen($class_strut[0]) + 1);
...
function A($name, $app='@') {
    ...
    if(stripos($name, 'Api.') === 0) {
        ...
        $name = array_pop($array);
        $className = $name . 'Api';
        import($app . '.Control.' . implode('.', $array) . '.' . $className);
    } else if (strpos($name, '.')) {
        $array = explode('.', $name);
        $name = array_pop($array);
